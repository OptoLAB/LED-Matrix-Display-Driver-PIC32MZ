...
#define	FCLK_SLOW()		SD_SPI_GO_SLOW()	/* Set slow clock for card initialization (100k-400k) */
#define	FCLK_FAST()		SD_SPI_GO_FAST()	/* Set fast clock for generic read/write */
...
{
	SPIBUF = dat;			/* Initiate an SPI transaction */
	while (!SPISTATbits.SPIRBF) ;		/* Wait for end of the SPI transaction */
	return (BYTE)SPIBUF;	/* Get received byte */
}
...
{
	do {
		SPIBUF = *buff++;	/* Initiate an SPI transaction */
		while (!SPISTATbits.SPIRBF) ;	/* Wait for end of the SPI transaction */
		SPIBUF;			/* Discard received byte */
		SPIBUF = *buff++;
...
{
	do {
		SPIBUF = 0xFF;		/* Initiate an SPI transaction */
		while (!SPISTATbits.SPIRBF) ;	/* Wait for end of the SPI transaction */
		*buff++ = SPIBUF;	/* Get received byte */
		SPIBUF = 0xFF;
		while (!SPISTATbits.SPIRBF) ;
		*buff++ = SPIBUF;
	} while (cnt -= 2);
